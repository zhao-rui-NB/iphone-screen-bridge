...
void ADV7611_set_edid(){
    ...
    for(int i = 0; i < 128; i++){
        ...
        ADV7611_write_reg(ADV7611_EDID_ADDR, i, EDID_V3[i]);
    }
...
void ADV7611_Init(){
    ...
    ADV7611_write_reg(ADV7611_IO_ADDR, 0x19, (0x80 +  26 )   ); // LLC DLL phase // ok
    // ADV7611_write_reg(ADV7611_IO_ADDR, 0x19, (0x80 +  10 )   ); // LLC DLL phase // ok
...
void ADV7611_set_dll_phase(uint8_t phase) {
    if (phase > 63) {
        printf("Invalid DLL phase: %d. Must be 0-63.\n", phase);
        return;
    }
    ADV7611_write_reg(ADV7611_IO_ADDR, 0x19, 0x80 + phase); // LLC DLL phase
}
