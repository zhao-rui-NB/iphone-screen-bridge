...
void ADV7611_Print_Debug_Status(void) {
    uint8_t val, val2;
    uint16_t data16;
...
void ADV7611_Init(){
    ...
    printf("Start reading ADV7611 registers...\n");
    // read reg 00
    ...
    // ADV7611_write_reg(ADV7611_IO_ADDR, 0x15, 0xb0); // m
    ...
    // ADV7611_write_reg(ADV7611_CP_ADDR, 0xba, 0x00); //  Disable the free run feature in HDMI mode
    // ADV7611_write_reg(ADV7611_HDMI_ADDR, 0x01, 0x01); //啟用自動終端控制 (TERM_AUTO)


    // 3. 【關鍵】喚醒晶片 (Power Up)
    // 關閉 Power Down Mode, 啟用晶片核心
    // ADV7611_write_reg(ADV7611_IO_ADDR, 0x0C, 0x42); 
    ...
    // 5. 推薦設定：HDMI 等化器 (Equalizer)
    // 對應長線材或訊號不穩很有幫助
    ADV7611_write_reg(ADV7611_HDMI_ADDR, 0x96, 0x01); // 啟用動態等化器 // 好像沒幫助
    
    // 6. 設定輸出格式 (這部分依您的需求，這裡是範例)
    // ADV7611_write_reg(ADV7611_IO_ADDR, 0x03, 0x40); // 24-bit SDR 4:4:4 Mode (範例)
    
    // 7. 最後：拉高 HPA (Hot Plug Assert) 讓電腦偵測
    // 確保以上設定都好了再通知電腦送訊號
    // ADV7611_write_reg(ADV7611_IO_ADDR, 0x20, 0xF0); // HPA_MANUAL=1, HPA_VALUE=1
